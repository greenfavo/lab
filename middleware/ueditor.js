...
var respond = function(static_url, callback) {
  return function(req, res, next) {
    if (req.query.action === 'config') {
      ...
      res.ue_list = function(list_dir) {
        ...
        fs.readdir(static_url + list_dir, function(err, files) {
          ...
          var filetype = 'zip,rar,pdf,doc,docx,xlsx,xls,ppt,pptx,txt,mp4,mp3';
          
          filetype+='jpg,jpeg,png,gif,psd';
    ...
    }else if (req.query.action === 'uploadimage'||'uploadfile') {
      ...
      busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
        ...
        res.ue_up = function(img_url) {
          ...
          if (req.query.action==='uploadimage') {
            ...
            //以原文件名+随机数命名
            var name= parseInt(Math.random()*100)+"-"+filename;
            req.ueditor.url=img_url+name;
